define(["app", "config"], function (CloudMamManager, config) {
    CloudMamManager.module("MyCloudApp.Dialog.Tree", function (Tree, CloudMamManager, Backbone, Marionette, $, _) {
        ...
        Tree.View = Backbone.View.extend({
            tagName           : 'li',
            template          : Marionette.TemplateCache.get("share/share-dialog-movetoform-file"),
            initialize        : function () {
            ...
            moveToFolder      : function (list) {//移动到文件夹
                ...
                list.targetFolder = selectedNode.get("code");  //要存放的目标文件夹
                if (selectedNode) {
                    var url = config.dcmpRESTfulIp + "/emc/share/saveToMe";
                    Backbone.ajax(url, {
                        type       : "POST",
                        data       : list,
